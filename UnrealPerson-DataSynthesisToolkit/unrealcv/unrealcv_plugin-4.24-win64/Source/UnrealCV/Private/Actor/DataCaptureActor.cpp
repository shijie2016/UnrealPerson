...
void ADataCaptureActor::CaptureImage()
{
	...
	for (ACamSensorActor* CameraActor : Sensors)
	{
		...
		TArray<UFusionCamSensor*> Sensorsx = CameraActor->GetSensors();

		if (SensorNames.Num() != Sensorsx.Num())
		{
			UE_LOG(LogTemp, Warning, TEXT("The number of CameraNames and Cameras are mismatch."));
			continue;
		}

		for (int i = 0; i < Sensorsx.Num(); i++)
		{
			this->CaptureImageFromSensor(SensorNames[i], Sensorsx[i]);
		}

	}
	//FString ScreenMessage = FString::Printf(TEXT("%d frames / %d images are captured from %d cameras"), FrameCounter, FrameCounter * Sensorsx.Num(), Sensorsx.Num());
